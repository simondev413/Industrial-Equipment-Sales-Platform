...
import { X, Mail, Lock, Wind, Loader2, UserPlus, ShieldCheck, User } from 'lucide-react';
import { toast } from 'sonner';
import { getStore, saveStore, useStore } from '@/app/lib/store';
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      const newUser = {
        ...
        avatar: `https://ui-avatars.com/api/?name=${regData.name}&background=random`,
        clientId: clientId
      };
      ...
      const newStore = { ...store, users: updatedUsers, clients: updatedClients };
      saveStore(newStore);
            ...
            <form onSubmit={handleRegister} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2 space-y-2">
                  <label className="text-sm font-semibold text-slate-700 ml-1">Nome da Empresa</label>
                  <input 
                    type="text" required
                ...
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-slate-700 ml-1">NIF</label>
                  <input 
                    type="text" required
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none"
                    value={regData.nif}
                ...
                <div className="col-span-2 space-y-2">
                  <label className="text-sm font-semibold text-slate-700 ml-1">Morada</label>
                  <input 
                    type="text" required
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none"
