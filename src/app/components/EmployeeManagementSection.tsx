...
import { ShieldCheck, UserPlus, Search, Mail, Trash2, Edit2 } from 'lucide-react';
...
export const EmployeeManagementSection = () => {
  ...
  const employees = store.users.filter((u: User) => u.role === 'employee' || u.role === 'admin');
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-500">Administração de funcionários e privilégios de acesso.</p>
        </div>
        <button 
      ...
      {showModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl p-8">
            <h2 className="text-xl font-bold text-slate-900 mb-6">Adicionar Colaborador</h2>
            <div className="space-y-4">
              ...
              <div>
                <label className="text-sm font-bold text-slate-700 block mb-2">Nível de Acesso</label>
                <select 
                  className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none"
